  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      const items = querySnap.docs.map((d) => {
        const raw = d.data() || {};
        const createdAt = raw.createdAt && raw.createdAt.toDate ? raw.createdAt.toDate() : null;
        return {
          id: d.id,
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow p-6">
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="p-4 rounded-lg bg-gray-50 dark:bg-slate-700/40 sm:col-span-2">
            ...
            {scores.length === 0 ? (
              <div className="mt-3 text-sm text-gray-600">No hay rondas registradas todavía.</div>
            ) : (
              <ul className="mt-3 space-y-2">
                {scores.map((s) => (
                  <li key={s.id} className="flex items-center justify-between p-2 rounded-md bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700">
                    <div>
        ...
        <div className="mt-6 flex gap-3">
          <button onClick={() => navigate("/")} className="px-4 py-2 bg-indigo-600 text-white rounded hover:cursor-pointer hover:bg-indigo-700">Jugar otra ronda</button>
          <button onClick={() => navigate("/")} className="px-4 py-2 border border-gray-300 rounded text-sm hover:cursor-pointer hover:bg-gray-100 dark:text-white dark:hover:bg-slate-700 dark:border-gray-600">Volver al inicio</button>
        </div>
      </div>
    </div>
  );
